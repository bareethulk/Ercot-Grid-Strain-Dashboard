...
with st.sidebar:
    ...
    st.metric("Strain Threshold", f"{threshold:,.0f} MWh")
...
with tab2:
    ...
    with col1:
        ...
        # Add trendline
        z = np.polyfit(filtered_df["Temperature_C"].dropna(), 
                      filtered_df.loc[filtered_df["Temperature_C"].notna(), "Total_Demand_MWh"], 2)
        p = np.poly1d(z)
        x_trend = np.linspace(filtered_df["Temperature_C"].min(), 
                             filtered_df["Temperature_C"].max(), 100)
